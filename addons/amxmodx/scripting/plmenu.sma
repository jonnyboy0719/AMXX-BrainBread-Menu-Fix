...
displayBanMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "BAN_MENU", pos + 1, (g_menuPlayersNum[id] / 7 + ((g_menuPlayersNum[id] % 7) ? 1 : 0)))
	new end = start + 7
...
displaySlapMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "SLAP_SLAY_MENU", pos + 1, (g_menuPlayersNum[id] / 7 + ((g_menuPlayersNum[id] % 7) ? 1 : 0)))
	new end = start + 7
	new keys = MENU_KEY_0|MENU_KEY_8
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (!is_user_alive(i) || (access(i, ADMIN_IMMUNITY) && i != id))
		{
			...
			if (g_coloredMenus)
				len += format(menuBody[len], 511-len, "#\%d. %s\R%s^n\w", b, name, team)
			else
			...
			if (is_user_admin(i))
				len += format(menuBody[len], 511-len, g_coloredMenus ? "#\%d. %s \r*\y %s^n\w" : "#\%d. %s *   %s^n", ++b, name, team)
			else
				len += format(menuBody[len], 511-len, g_coloredMenus ? "#\%d. %s\y %s^n\w" : "#\%d. %s   %s^n", ++b, name, team)
		}
	}
...
displayKickMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "KICK_MENU", pos + 1, (g_menuPlayersNum[id] / 8 + ((g_menuPlayersNum[id] % 8) ? 1 : 0)))
	new end = start + 8
	new keys = MENU_KEY_0
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (access(i, ADMIN_IMMUNITY) && i != id)
		{
			...
			if (g_coloredMenus)
				len += format(menuBody[len], 511-len, "#\%d. %s^n\w", b, name)
			else
				len += format(menuBody[len], 511-len, "#. %s^n", name)
		} else {
			keys |= (1<<b)
...
displayTeamMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "TEAM_MENU", pos + 1, (g_menuPlayersNum[id] / 7 + ((g_menuPlayersNum[id] % 7) ? 1 : 0)))
	new end = start + 7
	new keys = MENU_KEY_0|MENU_KEY_8
	...
	for (new a = start; a < end; ++a)
	{
		...
		if ((iteam == g_CSTeamiNumbers[g_menuOption[id] % (g_cstrike ? 3 : 2)]) || (access(i, ADMIN_IMMUNITY) && i != id))
		{
			...
			if (g_coloredMenus)
				len += format(menuBody[len], 511-len, "#\%d. %s %s^n\w", b, name, team)
			else
				len += format(menuBody[len], 511-len, "#. %s   %s^n", name, team)		
		} else {
			keys |= (1<<b)
...
displayClcmdMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "CL_CMD_MENU", pos + 1, (g_menuPlayersNum[id] / 7 + ((g_menuPlayersNum[id] % 7) ? 1 : 0)))
	new end = start + 7
	new keys = MENU_KEY_0|MENU_KEY_8
