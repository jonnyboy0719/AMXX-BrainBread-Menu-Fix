...
displayVoteMapsMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "VOTEMAP_MENU", pos + 1, (g_mapNums / 7 + ((g_mapNums % 7) ? 1 : 0)))
	...
	for (new a = start; a < end; ++a)
	{
		...
		if (g_voteSelectedNum[id] == 4 || isMapSelected(id, pos * 7 + b))
		{
			++b
			if (g_coloredMenus)
				len += format(menuBody[len], 511-len, "#\%d. %s^n\w", b, tempMap)
			else
...
displayMapsMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, "CHANGLE_MENU", pos + 1, (g_mapNums / 8 + ((g_mapNums % 8) ? 1 : 0)))
	new end = start + 8
	new keys = MENU_KEY_0
