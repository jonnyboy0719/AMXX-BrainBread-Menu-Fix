...
displayMenu(id, pos)
{
	...
	g_coloredMenus ? "\yAMX Mod X Menu %d/%d^n\w^n" : "AMX Mod X Menu %d/%d^n^n" , pos + 1, (g_menusNumber / MENUITEMSPERPAGE) + (((g_menusNumber % MENUITEMSPERPAGE) > 0) ? 1 : 0))
			 ...
			 (is_plugin_loaded(g_menuPlugin[a], true) != -1)))	// search plugins for filename
		{
			...
			if (g_coloredMenus)
			{
				if (g_menuBodyPhrase[a])
					...
					len += format(menuBody[len], 511-len, "#\%d. %s^n\w", b, g_menuBody[a])
			} else {
				if (g_menuBodyPhrase[a])
...
clientDisplayMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\yAMX Mod X Client Menu %d/%d^n\w^n" : "AMX Mod X Client Menu %d/%d^n^n" , pos + 1, (g_clientMenusNumber / MENUITEMSPERPAGE) + (((g_clientMenusNumber % MENUITEMSPERPAGE) > 0) ? 1 : 0))
	...
	for (new a = start; a < end; ++a)
	{
			 ...
			 (is_plugin_loaded(g_clientMenuPlugin[a], true) != -1)))		// search plugins for file name
		{
			...
			{
				if (g_clientMenuBodyPhrase[a])
					len += format(menuBody[len], 511-len, "#\%d. %L^n\w", b, id, g_clientMenuBody[a])
				else
					len += format(menuBody[len], 511-len, "#\%d. %s^n\w", b, g_clientMenuBody[a])
			} else {
				if (g_clientMenuBodyPhrase[a])
