...
displayCmdMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\y%L %d/%d^n\w^n" : "%L %d/%d^n^n", id, g_cmdMenuName[g_menuLayer[id]], pos + 1, (limit == 0) ? 1 : limit)
	new end = start + 8
	new keys = MENU_KEY_0
...
displayCvarMenu(id, pos)
{
	...
	new len = format(menuBody, 511, g_coloredMenus ? "\yCvars Menu %d/%d^n\w^n" : "Cvars Menu %d/%d^n^n", pos + 1, (g_menuSelectNum[id] / 8 + ((g_menuSelectNum[id] % 8) ? 1 : 0)))
